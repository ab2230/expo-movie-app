  ...
  return (
    <View className='bg-primary flex-1'>
      <ScrollView contentContainerStyle={{
        ...
        <View>
          <Image source={{  uri: movie?.poster_path? process.env.EXPO_PUBLIC_IMAGE_BASE_URL+movie?.poster_path:
                process.env.EXPO_PUBLIC_IMAGE_PLACEHOLDER_URL}}
        ...
        <View className='flex-col items-start justify-center mt-5 px-5'>
            ...
            <MovieInfo label="Genres" value={movie?.genres?.map((g) => g.name).join(' - ')||"N/A"}/>
              <View className='flex flex-row justify-between w-1/2'>
                <MovieInfo label='Budget' value={`$${movie?.budget && movie?.budget/ 1_000_000} million`} />
                <MovieInfo label='Revenue' value={`$${movie?.revenue && Math.round(movie?.budget/ 1_000_000)} million`} />
